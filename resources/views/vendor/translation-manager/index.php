    ...
    <script>
        jQuery(document).ready(function($){
            ...
            $('.group-select').on('change', function(){
                var group = $(this).val();
                if (group) {
                    window.location.href = '<?php echo action('\App\Http\Controllers\TranslateController@getView') ?>/'+$(this).val();
                } else {
                    window.location.href = '<?php echo action('\App\Http\Controllers\TranslateController@getIndex') ?>';
                }
            });
...
<?php endif; ?>
    ...
    <?php if($group): ?>
        <form action="<?php echo action('\App\Http\Controllers\TranslateController@postAdd', array($group)) ?>" method="POST"  role="form">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <div class="form-group">
                <label>Добавить новый ключ в данную группу</label>
        ...
        <table class="table">
            ...
            <?php foreach ($translations_ as $key => $translation): ?>
                <tr id="<?php echo htmlentities($key, ENT_QUOTES, 'UTF-8', false) ?>">
                    ...
                    <?php if ($deleteEnabled): ?>
                        <td>
                            <a href="<?php echo action('\App\Http\Controllers\TranslateController@postDelete', [$group, $key]) ?>"
                               class="delete-key"
    ...
    <?php else: ?>
        <fieldset>
            ...
            <!--            </p>-->
            <!--            <form  class="form-remove-locale" method="POST" role="form" action="--><?php //echo action('\App\Http\Controllers\TranslateController@postRemoveLocale') ?><!--" data-confirm="Are you sure to remove this locale and all of data?">-->
            <!--                <input type="hidden" name="_token" value="--><?php //echo csrf_token(); ?><!--">-->
            ...
            <!--                </ul>-->
            <!--            </form>-->
            <!--            <form class="form-add-locale" method="POST" role="form" action="--><?php //echo action('\App\Http\Controllers\TranslateController@postAddLocale') ?><!--">-->
            <!--                <input type="hidden" name="_token" value="--><?php //echo csrf_token(); ?><!--">-->
            <!--                <div class="form-group">-->
            <!--                    <p>-->
        ...
        <!--            <form class="form-inline form-publish-all" method="POST" action="--><?php //echo action('\App\Http\Controllers\TranslateController@postPublish', '*') ?><!--" data-remote="true" role="form" data-confirm="Are you sure you want to publish all translations group? This will overwrite existing language files.">-->
        <!--                <input type="hidden" name="_token" value="--><?php //echo csrf_token(); ?><!--">-->
