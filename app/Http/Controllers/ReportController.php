...
//        return response()->json($result);
    ...
    public function getReport112BranchesExport(Request $request)
    {
        ...
        foreach ($cards->get() as $card) {
            ...
            if($card->additionalIncident->name == 'Падение веток и деревьев') {
                $preparedToExport[$card->cityArea->name][] = [
                    '№' => $card->id,
                    'Адрес' => $card->location,
        ...
        $title = "Информация по подтоплениям и падениям веток/деревьев за пероид c {$dateStartHuman}. по {$dateEndHuman}г., зафиксировано {$cards->count()}";
        ...
        if($incidentType->name === 'Подтопления') {
            ...
            foreach ($preparedToExport as $key => $data) {
                ...
                $t = CityArea::find($cityAreaId)->name ?? null;
...
//            })
    ...
    public function queuedReportsView()
    {
        return view('reports.queued-reports-view');
    }
}
