...
class Ticket112EmergencyExcelExport
{
    ...
    private function prepareSpreadsheet()
    {
        ...
        $sheet->setTitle('Чрезвычайные ситуации');
    ...
    private function addFirstTableHeaders(Worksheet $sheet)
    {
        // заголовки таблицы
        $sheet->getCell('B1')
            ->setValue("Чрезвычайные ситуации природного и техногенного характера зарегистрированные по г. Алматы за {$this->data['dateFrom']} по {$this->data['dateTo']}");
    ...
    private function addFirstTableData(Worksheet $sheet, $startRowIndex = 3)
    {
        ...
        foreach ($this->data['values'] as $rowVal) {
            ...
            $sheet->getStyle("A$rowIndex:H$rowIndex")->applyFromArray(self::HStyle);
