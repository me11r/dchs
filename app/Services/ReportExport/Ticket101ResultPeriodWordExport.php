...
class Ticket101ResultPeriodWordExport
{
    ...
    private function addFirstTable(Section $section)
    {
        ...
        $section->addText('Итого - '.$this->data['total'],
    ...
    {
        foreach ($this->data['records'] as $rowVal) {
            $row = $table->addRow();
    ...
    private function addFirstTableHeaders(\PhpOffice\PhpWord\Element\Table $table)
    {
        ...
        foreach (['Наименование', 'Количество'] as $header) {
