...
class Ticket101ObjectClassExcelExport
{
    ...
    private function prepareSpreadsheet()
    {
        ...
        $sheet->setTitle('Классификация объектов');
    ...
    private function addFirstTableHeaders(Worksheet $sheet)
    {
        ...
        foreach ($this->data['values']['ПТЗ']['headers'] as $index => $header) {
    ...
    private function addSecondTableHeaders(Worksheet $sheet)
    {
        ...
        foreach ($this->data['values']['ПТУ']['headers'] as $index => $header) {
            $sheet->getColumnDimension($letters[$index])->setWidth(20);
            $this->setCell($sheet, $header, "$letters[$index]16", "$letters[$index]16", self::HStyle);
        }
    ...
    private function addFirstTableData(Worksheet $sheet, $startRowIndex = 3)
    {
        ...
        foreach ($this->data['values']['ПТЗ']['values'] as $rowVal) {

            $sheet->fromArray($rowVal, null, "A$rowIndex");
            $sheet->getStyle("A$rowIndex:N$rowIndex")->applyFromArray(self::HStyle);
    ...
    private function addSecondTableData(Worksheet $sheet, $startRowIndex = 17)
    {
        $rowIndex = $startRowIndex;

        foreach ($this->data['values']['ПТУ']['values'] as $rowVal) {
