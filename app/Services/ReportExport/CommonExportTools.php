...
//        $delimiter = '<w:br/>';
        ...
        return [
            ...
            $department->tech_action ? implode($delimiter, $department->tech_action->pluck('vehicle.name')->toArray()) : '', // Пожарная техника ->  В боевом расчёте -> Тип основ пожарного а/м
            $department->tech_action ? implode($delimiter, $department->tech_action->where('vehicle.vehicle_type_id', '=', 2)->pluck('vehicle.base')->toArray()) : '', // Пожарная техника ->  В боевом расчёте -> Марка спец. пожарного а/м Мотоциклы

            $department->tech_reserve ? implode($delimiter, $department->tech_reserve->pluck('vehicle.name')->toArray()) : '', // Пожарная техника ->  В резерве -> Тип основ пожарного а/м
            $department->tech_reserve ? implode($delimiter, $department->tech_reserve->where('vehicle.vehicle_type_id', '=', 2)->pluck('vehicle.base')->toArray()) : '', // Пожарная техника ->  В резерве -> Марка спец. пожарных а/м

            $department->tech_repair ? implode($delimiter, $department->tech_repair->pluck('vehicle.name')->toArray()) : '', // Пожарная техника ->  На ремонте -> Тип основ пожарного а/м
            $department->tech_repair ? implode($delimiter, $department->tech_repair->where('vehicle.vehicle_type_id', '=', 2)->pluck('vehicle.base')->toArray()) : '', // Пожарная техника ->  На ремонте -> Марка спец. пожарных а/м
        ];
    }
