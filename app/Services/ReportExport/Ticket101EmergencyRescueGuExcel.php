...
class Ticket101EmergencyRescueGuExcel
{
    ...
    private function addFirstTableHeaders(Worksheet $sheet)
    {
        // заголовки таблицы
        $sheet->getCell('B1')
            ->setValue("Учет аварийно-спасательных работ ГУ СПиАСР за {$this->data['dateFrom']} по {$this->data['dateTo']}");
    ...
    private function addFirstTableData(Worksheet $sheet, $startRowIndex = 4)
    {
        ...
        $letters = range('A', 'Z');
        foreach ($this->data['values'] as $index => $header) {
            $sheet->getColumnDimension($letters[$index])->setWidth(20);
