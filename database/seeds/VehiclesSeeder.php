...
//        \App\Models\Vehicle::where('id', '>', 0)->delete();
    ...
    public function getData()
    {
        ...
        foreach ($raw_data as $raw_datum) {
            ...
            if($temp_item[5] != null){
                $temp_item[5] = str_replace(' ', '', $temp_item[5]);
            }
            ...
            $changed_keys['number_old'] = strlen($temp_item[5]) < 4 ? null : trim($temp_item[5]);
        ...
        foreach ($raw_data_less as $key => $item) {
            ...
            if($fire_dep_id){
                ...
                if(str_contains($item['vehicle_type_id'], $vehicle_types[0])){
                    $vehicle_type = \App\Models\VehicleType::where('name', 'like', "$vehicle_types[0]%")->first();
                }
