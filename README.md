...
Кроме этого, в ```database/dumps/``` лежит файл в ```functions.sql``` в котором находится функция бд, без которой не будет работать строевые записки.
Так же там же лежит полный последний дамп на 12.02.2021 с расширением ```*.tar.gz```, распаковать и залить в базу удобным способом.
